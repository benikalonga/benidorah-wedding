...
if (!ADMIN_SEED_PASSWORD) {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('ADMIN_SEED_PASSWORD must be set (in .env) before seeding a production database.');
  }
  console.warn('[seed] ADMIN_SEED_PASSWORD not set — using a dev-only fallback. Set it in .env for anything beyond local dev.');
}
...
const HISTORY_SEEDS = [
  ...
  {
    title: 'First Date',
    descriptionShort: 'Coffee turned into dinner.',
    descriptionFull: 'What was meant to be a quick coffee stretched into a four-hour dinner. We knew this was the start of something real.',
    eventDate: new Date('2021-04-02'),
  },
  {
    title: 'Made It Official',
    descriptionShort: "We said yes to 'us'.",
    descriptionFull: 'After months of getting to know each other, we decided to stop wondering and start building something together.',
    eventDate: new Date('2021-08-20'),
  },
  {
    title: 'Moving In Together',
    descriptionShort: 'Our first home.',
    descriptionFull: 'We found a little apartment that became the backdrop for countless memories, arguments over furniture, and quiet mornings.',
    eventDate: new Date('2023-01-10'),
  },
  {
    title: 'The Proposal',
    descriptionShort: 'He asked. She said yes.',
    descriptionFull: 'On a rooftop overlooking the city, with the sun setting behind us, Beni got down on one knee — and Dorah said yes before he could finish the question.',
    eventDate: new Date('2025-06-15'),
  },
  {
    title: 'Meeting the Families',
    descriptionShort: 'Two families, one love.',
    descriptionFull: 'Sunday lunches and long conversations brought our families together long before the wedding planning began.',
    eventDate: new Date('2025-07-01'),
  },
  {
    title: 'The Engagement Shoot',
    descriptionShort: 'Capturing the moment.',
    descriptionFull: 'Dressed in royal blue and classic black-tie, we spent an afternoon on a rooftop terrace capturing this chapter of our story.',
    eventDate: new Date('2025-09-05'),
  },
  {
    title: 'Save the Date',
    descriptionShort: 'Telling the world.',
    descriptionFull: 'We finally shared the news with everyone we love — 23 December 2026 is the day we say "I do."',
    eventDate: new Date('2025-10-01'),
  },
  {
    title: 'Wedding Planning Begins',
    descriptionShort: 'Venues, vendors, vision boards.',
    descriptionFull: 'From venue visits to menu tastings, every detail has been chosen with love for the day ahead.',
    eventDate: new Date('2026-02-01'),
  },
  {
    title: 'The Wedding Day',
    descriptionShort: 'Forever begins.',
    descriptionFull: 'The day we become husband and wife, surrounded by everyone who has walked this journey with us.',
...
async function main() {
  ...
  for (const seed of ADMIN_SEEDS) {
    await prisma.adminUser.upsert({
      ...
      update: {},
      create: { email: seed.email, name: seed.name, passwordHash, mustChangePassword: true, role: 'admin' },
    });
  }
  ...
  const coupleImages = ['/images/couple/1.jpg', '/images/couple/2.jpg', '/images/couple/3.jpg'];
  for (let i = 0; i < HISTORY_SEEDS.length; i++) {
    const h = HISTORY_SEEDS[i];
    const existing = await prisma.historyItem.findFirst({ where: { title: h.title } });
    if (!existing) {
  ...
  if (galleryCount === 0) {
    await prisma.galleryItem.createMany({
      data: coupleImages.map((url, i) => ({ mediaUrl: url, mediaType: 'image' as const, sortOrder: i })),
    });
  }
  ...
  const highlightInGallery = await prisma.galleryItem.findFirst({ where: { mediaUrl: '/api/media/highlight' } });
  ...
  const galleryBatch: { url: string; type: 'image' | 'video'; thumbnailUrl?: string }[] = [];
  for (let n = 1; n <= 26; n++) {
    if (n === 3) {
      galleryBatch.push({ url: '/images/gallery/3.mp4', type: 'video', thumbnailUrl: '/images/gallery/3-poster.jpg' });
    } else {
      galleryBatch.push({ url: `/images/gallery/${n}.jpeg`, type: 'image' });
    }
  }
  for (const item of galleryBatch) {
    const existing = await prisma.galleryItem.findFirst({ where: { mediaUrl: item.url } });
